...
The section below reflects the current unreleased state of the repository as of 2026-03-11.
...
- a dedicated UI regression test covering horizontal scrolling on the iOS 17+ `Charts` path.
- a short team-facing migration brief in `MIGRATION.md`.
...
- public API guidance now prefers:
  - `visibleValueCount`
...
- the default runtime model is now explicit:
  - iOS 17+: `Charts + Apple Charts scroll`
  - iOS 16: `Canvas + SwiftUI Gesture`
  - UIKit scroll remains fallback / diagnostics only
- the package now declares iOS-only support; macOS is not supported.
- `Charts` selection overlays no longer block horizontal scrolling on iOS 17+.
- viewport, selection, overlay, and debug semantics have been further unified across renderer paths.
- sample launch flags now prefer:
  - `-snapshot-scroll-engine`
  - `-snapshot-scroll-target-behavior`
  while still accepting the legacy flags for compatibility.
...
- downstream code and tooling should migrate toward:
  - `visibleValueCount`
  - `scrollTargetBehavior`
  - `scrollEngine`
  - `startIndex`
